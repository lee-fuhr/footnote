...
function _renderClusters(sheet, clusters, processedAt) {
  ...
  container.innerHTML = clusters.map(c => `
    <div class="insights-cluster-card" data-id="${c.id}">
  ...
  if (hoursSince > 48) footer.classList.add('insights-stale')
  else footer.classList.remove('insights-stale')
}
...
async function _loadAndShow(sheet) {
  ...
  // Check if enough walks to analyze
  ...
  // Have enough walks, auto-analyze if today not yet processed
  const today = new Date().toISOString().slice(0, 10)
  const lastProcessed = await getMeta('walkInsightsProcessedDate')
  const consent = await getMeta('insightsConsentGiven')

  if (lastProcessed === today) {
    // Already processed today but no clusters — show empty
    _setState(sheet, 'empty')
    return
  }

  if (!consent) {
    // Show consent sheet before auto-analyzing
...
async function _runAnalysis(sheet) {
  ...
  try {
    ...
    _showError(sheet, err.message === 'daily budget exceeded'
      ...
      : 'Couldn\'t connect to the AI. Check your connection and try again.')
  }
}
...
async function _openSheet() {
  const { sheet, analyzeBtn } = _ensureEl()
